...
const createComments = (commentData) => {
    ...
    commentData.forEach((comment) => {
        ...
        const paraElement = createElemWithText('p', comment.body);
        const paraElement2 = createElemWithText('p', `From: ${comment.email}`);
        articleElement.append(h3Element, paraElement, paraElement2);
...
const populateSelectMenu = (userData) => {
    ...
    const optionElements = createSelectOptions(userData);
    optionElements.forEach((option) => {
        selectMenuElement.append(option);
    });
...
const createPosts = async(jsonPostData) => {
    if(!(jsonPostData)) {
        return undefined;
    }
    ...
    for(const post of jsonPostData) {

        const newArticle = document.createElement("article");
        const newH2 = await createElemWithText("h2", post.title);
        const newP = await createElemWithText("p", post.body);
        const newP2 = await createElemWithText("p", `Post ID: ${post.id}`);
        ...
        newArticle.append(newH2, newP, newP2, newP3, newP4, myButton);
        
        const section = await displayComments(post.id);
        newArticle.append(section);

        newFragment.append(newArticle);
    };
...
const toggleComments = (event, postId) => {
    ...
    const myCommentButton = toggleCommentButton(postId);
    return [mySection, myCommentButton];
};
